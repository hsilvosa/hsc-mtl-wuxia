...
def load_model_resources(config: Config):
    ...
    tokenizer.padding_side = "left" # Correcto for decoder-only

    # If not there are pad_token defined, use eos_token (common in Llama/Hunyuan)
...
def translate_batch_hunyuan(
    ...
    """
    Function vectorizada adaptada for Hunyuan-MT (Fix: token_type_ids).
    """
    prompts_as_strings = []

    # 1. Prepare prompts individual
    ...
    # === FIX CRITICAL ===
    # Remove token_type_ids if exists, already that Hunyuan not it supports
        ...
        for prompt_idx in PROMPT_IDS:
            ...
            for mode_name, n_shots in SHOTS_CONFIG.items():
                ...
                for i in tqdm(batch_starts, desc=progress_desc):
                    ...
                    try:
                        # 2. Inference in Batch (Using the function adaptada)
