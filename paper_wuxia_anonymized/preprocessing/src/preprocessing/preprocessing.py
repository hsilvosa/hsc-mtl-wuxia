...
# List of score to keep (can adjust it if want fewer)
ALLOWED_PUNCT = set(string.punctuation) | {"，", "。", "！", "？", "、", "；", "：", "“", "”", "‘", "’", "—", "…"}

def is_letter_or_mark(ch: str) -> bool:
    # Letters and marks (pinyin with diacritics, characters Chinese)
...
def main():
    ...
    if len(df) < 10:
        raise ValueError("The dataset has very few lines after the cleaning.")
    ...
    print(f"Saved DatasetDict in: {out_dir}")
